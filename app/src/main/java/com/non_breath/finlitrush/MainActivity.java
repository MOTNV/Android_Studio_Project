...
public class MainActivity extends AppCompatActivity {
    ...
    private void updateRecommendations(AnalysisResult result) {
        ...
        List<String> recList = result.getRecipients();
        if (recList != null && !recList.isEmpty()) {
            for (String name : recList) {
                ProfessorDirectory.Professor p = findByName(all, name);
                if (p != null && !picks.contains(p)) {
                    picks.add(p);
                    if (picks.size() >= MAX_REC) break;
                }
            }
        }
            ...
            if (keywords != null && !keywords.isEmpty()) {
                ...
                for (String kw : keywords) {
                    if (kw != null && !kw.trim().isEmpty()) {
                        filtered.add(kw.trim());
                    }
                }
                for (ProfessorDirectory.Professor p : all) {
                    if (picks.size() >= MAX_REC) break;
                    if (matchesKeywords(p, filtered)) {
                        picks.add(p);
                    }
                }
            }
    ...
    private ProfessorDirectory.Professor findByName(List<ProfessorDirectory.Professor> all, String name) {
        ...
        for (ProfessorDirectory.Professor p : all) {
            String base = normalizeName(p.name);
    ...
    private boolean matchesKeywords(ProfessorDirectory.Professor p, List<String> keywords) {
        ...
        for (String kw : keywords) {
            ...
            if (containsAlias(low, fields, "nlp", "자연어처리")) return true;
            if (containsAlias(low, fields, "database", "데이터베이스", "db")) return true;
            if (containsAlias(low, fields, "vision", "컴퓨터비전")) return true;
